...
import { Loader2, List, Shield, CheckCircle2, XCircle, ShieldAlert, ShieldClose, ShieldCheck, Hourglass, Search, Trash2 } from "lucide-react";
...
export function LicenseList() {
    ...
    const handleDelete = async () => {
        ...
        setActionLoading(`delete-${licenseToDelete.id}`);
        try {
            const response = await fetch("/api/admin/delete_license", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${adminKey}`,
                },
                body: JSON.stringify({ clientId: licenseToDelete.id }),
            });
        ...
        <Card className="shadow-lg col-span-1 lg:col-span-2">
                    ...
                    <TooltipProvider>
                                    ...
                                    {filteredLicenses.map((license) => {
                                        ...
                                        return (
                                            <TableRow key={license.id} className={license.status === 'PENDIENTE' ? 'bg-blue-500/5' : ''}>
                                                <TableCell>
                                                    <div className="font-medium">{license.name || 'Unnamed'}</div>
                                                    <div className="text-sm text-muted-foreground font-mono">{license.id}</div>
                                                </TableCell>
                                                <TableCell>
                                                    ...
                                                    {license.last_seen ? (
                                                        <Tooltip>
                                                          <TooltipTrigger asChild>
                                                            <span>{formatDistanceToNow(new Date(license.last_seen), { addSuffix: true, locale: es })}</span>
                                                          </TooltipTrigger>
                                                          <TooltipContent>
                                                            <p>{new Date(license.last_seen).toLocaleString()}</p>
                                                          </TooltipContent>
                                                        </Tooltip>
                                                    ) : (
                                                    ...
                                                    {license.status === 'PENDIENTE' && (
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                ...
                                                                >
                                                                    {actionLoading === `status-${license.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4 text-success" />}
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                                <p>Approve License</p>
                                                    ...
                                                    {license.status === 'ACTIVA' && (
                                                         <Tooltip>
                                                            <TooltipTrigger asChild>
                                                                <Button 
                                                                    ...
                                                                    disabled={actionLoading === `status-${license.id}` || isExpired}
                                                                >
                                                                    {actionLoading === `status-${license.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldClose className="h-4 w-4 text-destructive" />}
                                                                </Button>
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                                <p>Block License</p>
                                                     ...
                                                     <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            ...
                                                            >
                                                                {actionLoading === `delete-${license.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4 text-destructive" />}
                                                            </Button>
                                                        </TooltipTrigger>
                                                        <TooltipContent>
                                                            <p>Delete License</p>
                        ...
                        {!isLoading && !error && licenses.length === 0 && hasSearched && (
                             <div className="flex flex-col justify-center items-center h-full text-center p-8">
                                ...
                                <p className="text-sm text-muted-foreground">The database is currently empty or no licenses match your search.</p>
                            </div>
                        )}
                        
                        {!isLoading && !error && filteredLicenses.length === 0 && searchTerm && (
                             <div className="flex flex-col justify-center items-center h-full text-center p-8">
                                <p className="font-medium">No Matching Licenses</p>
                                <p className="text-sm text-muted-foreground">No licenses found for "{searchTerm}".</p>
                            </div>
                        )}
            ...
            <AlertDialog open={!!licenseToDelete} onOpenChange={(open) => !open && setLicenseToDelete(null)}>
                <AlertDialogContent>
                    ...
                    <AlertDialogDescription>
                        This action cannot be undone. This will permanently delete the license for client <span className="font-mono font-bold">{licenseToDelete?.id}</span>.
                    </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={handleDelete} className="bg-destructive hover:bg-destructive/90">
                        {actionLoading === `delete-${licenseToDelete?.id}` && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                        Delete
                    </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
